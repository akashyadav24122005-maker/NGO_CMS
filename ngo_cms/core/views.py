...
from .models import (
    ...
    Donation, Volunteer, ContactMessage
)
from .forms import (
    ContactForm, VolunteerForm, BannerForm, InitiativeForm,
    ProjectForm, BlogPostForm, PressReleaseForm,
    MediaCoverageForm, GalleryImageForm, VideoForm,
    OurStoryForm, CoreValueForm, ProgramForm, TeamMemberForm,
    VisionMissionForm, StatisticForm
)
...
def edit_story(request, pk):
    ...
    if form.is_valid():
        ...
        messages.success(request, 'Story updated!')
        return redirect('manage_about')
    return render(request, 'dashboard/about_edit_form.html', {'form': form, 'title': 'Edit Our Story'})
...
def edit_core_value(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect('manage_about')
    return render(request, 'dashboard/about_edit_form.html', {'form': form, 'title': 'Edit Core Value'})
...
def edit_program(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect('manage_about')
    return render(request, 'dashboard/about_edit_form.html', {'form': form, 'title': 'Edit Program'})
...
def edit_team_member(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect('manage_about')
    return render(request, 'dashboard/about_edit_form.html', {'form': form, 'title': 'Edit Team Member'})
...
def projects_list(request):
    ...
    if status_filter in ['Ongoing', 'Completed', 'Upcoming']:
        projects = Project.objects.filter(is_active=True, status=status_filter)
    else:
...
def manage_projects(request):
    ...
    return render(request, 'dashboard/project_manage.html', {'projects': projects, 'form': form})
...
def manage_media(request):
    pr_form = PressReleaseForm()
    mc_form = MediaCoverageForm()
    gi_form = GalleryImageForm()
    v_form = VideoForm()

    if request.method == 'POST':
        section = request.POST.get('section')
        if section == 'press_release':
            pr_form = PressReleaseForm(request.POST)
            if pr_form.is_valid():
                pr_form.save()
                messages.success(request, 'Press release added!')
                return redirect('manage_media')
        elif section == 'media_coverage':
            mc_form = MediaCoverageForm(request.POST)
            if mc_form.is_valid():
                mc_form.save()
                messages.success(request, 'Media coverage added!')
                return redirect('manage_media')
        elif section == 'gallery':
            gi_form = GalleryImageForm(request.POST, request.FILES)
            if gi_form.is_valid():
                gi_form.save()
                messages.success(request, 'Image added!')
                return redirect('manage_media')
        elif section == 'video':
            v_form = VideoForm(request.POST)
            if v_form.is_valid():
                v_form.save()
                messages.success(request, 'Video added!')
                return redirect('manage_media')

    return render(request, 'dashboard/media_manage.html', {
        'pr_form': pr_form,
        'mc_form': mc_form,
        'gi_form': gi_form,
        'v_form': v_form,
...
def delete_press_release(request, pk):
    get_object_or_404(PressRelease, pk=pk).delete()
    messages.success(request, 'Press release deleted!')
    return redirect('manage_media')


@login_required
def delete_media_coverage(request, pk):
    get_object_or_404(MediaCoverage, pk=pk).delete()
...
@login_required
def edit_press_release(request, pk):
    obj = get_object_or_404(PressRelease, pk=pk)
    form = PressReleaseForm(request.POST or None, instance=obj)
    if form.is_valid():
        form.save()
        messages.success(request, 'Press release updated!')
        return redirect('manage_media')
    return render(request, 'dashboard/media_edit_form.html', {'form': form, 'title': 'Edit Press Release'})


@login_required
def edit_media_coverage(request, pk):
    obj = get_object_or_404(MediaCoverage, pk=pk)
    form = MediaCoverageForm(request.POST or None, instance=obj)
    if form.is_valid():
        form.save()
        messages.success(request, 'Media coverage updated!')
        return redirect('manage_media')
    return render(request, 'dashboard/media_edit_form.html', {'form': form, 'title': 'Edit Media Coverage'})
...
def edit_gallery_image(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect('manage_media')
    return render(request, 'dashboard/media_edit_form.html', {'form': form, 'title': 'Edit Gallery Image'})


@login_required
def edit_video(request, pk):
    obj = get_object_or_404(Video, pk=pk)
    form = VideoForm(request.POST or None, instance=obj)
    if form.is_valid():
        form.save()
        messages.success(request, 'Video updated!')
        return redirect('manage_media')
    return render(request, 'dashboard/media_edit_form.html', {'form': form, 'title': 'Edit Video'})
...
def banner_add(request):
    if request.method == "POST":
        ...
        form = BannerForm()
    return render(request, "dashboard/banner_list.html", {"form": form, "banners": Banner.objects.all()})